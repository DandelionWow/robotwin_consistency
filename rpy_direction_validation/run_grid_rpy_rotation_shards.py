#!/usr/bin/env python3
"""Run rpy_grid_rpy_rotation in fixed-seed episode shards."""
...
DEFAULT_TASK_NAME = "rpy_grid_rpy_rotation"
DEFAULT_TASK_CONFIG = "rpy_grid_rpy_rotation_clean"
DEFAULT_EPISODE_NUM = 1728
...
DEFAULT_SAPIEN_OIDN_LIBRARY = (
    "/data1/liuwenhao/conda/envs/RoboTwin/lib/python3.10/site-packages/sapien/oidn_library"
)
...
def prepend_env_path(env: dict, key: str, path: str) -> None:
    if not Path(path).exists():
        return
    old_value = env.get(key, "")
    old_parts = [item for item in old_value.split(":") if item]
    if path not in old_parts:
        env[key] = f"{path}:{old_value}" if old_value else path
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--arm-filter", choices=["all", "left", "right", "both"], default="all")
...
def selected_episode_ids(args: argparse.Namespace) -> list[int]:
    start, end = episode_range(args)
    episode_ids = list(range(start, end))
    if args.arm_filter == "all":
        return episode_ids

    def keep(episode_id: int) -> bool:
        block_index = episode_id % 864
        if args.arm_filter == "left":
            return block_index < 324
        if args.arm_filter == "right":
            return 324 <= block_index < 648
        return 648 <= block_index < 864

    return [episode_id for episode_id in episode_ids if keep(episode_id)]


def worker_episode_ids(episode_ids: list[int], worker_index: int, worker_count: int) -> list[int]:
    start, end = worker_bounds(0, len(episode_ids), worker_index, worker_count)
    return episode_ids[start:end]
...
def setup_worker_environment(args: argparse.Namespace) -> None:
    ...
    os.environ["MPLCONFIGDIR"] = "/data1/liuwenhao/tmp/mplconfig"
    os.environ["XDG_CACHE_HOME"] = "/data1/liuwenhao/tmp/xdg-cache"
    ...
    prepend_env_path(os.environ, "LD_LIBRARY_PATH", DEFAULT_SAPIEN_OIDN_LIBRARY)
...
def run_one_episode(cd, task_name: str, base_args: dict, episode_idx: int, seed: int, resume: bool) -> dict:
    ...
    meta_path = save_path / "meta" / f"episode_{episode_idx:06d}.json"
    if resume and hdf5_path.exists() and meta_path.exists():
    ...
    try:
        ...
        if not (env.plan_success and env.check_success()):
            return {"episode_idx": episode_idx, "skipped": False, "success": False, "phase": "plan"}
...
def run_worker(args: argparse.Namespace) -> int:
    ...
    save_path = Path(base_args["save_path"])
    all_episode_ids = selected_episode_ids(args)
    if args.worker_index is not None and args.worker_count is not None:
        episode_ids = worker_episode_ids(all_episode_ids, args.worker_index, args.worker_count)
    else:
        episode_ids = all_episode_ids

    print(
        f"[rpy_grid_rpy_rotation_shard] worker={args.worker_index} "
        f"episodes={len(episode_ids)}/{len(all_episode_ids)} arm_filter={args.arm_filter} "
        f"gpu={args.gpu} seed={args.seed}"
    )
    ...
    for episode_idx in episode_ids:
        try:
            ...
            if not result.get("skipped") and not result.get("success"):
                ...
                print(f"[rpy_grid_rpy_rotation_shard] episode={episode_idx} failed: {result}")
            else:
                print(f"[rpy_grid_rpy_rotation_shard] episode={episode_idx} done skipped={result.get('skipped')}")
        except Exception as exc:
            failure = {"episode_idx": episode_idx, "success": False, "error": repr(exc)}
            results.append(failure)
            failures.append(failure)
            print(f"[rpy_grid_rpy_rotation_shard] episode={episode_idx} exception={exc!r}")
            time.sleep(1)
...
def run_parent(args: argparse.Namespace) -> int:
    ...
    for worker_index in range(args.workers):
        ...
        env["MPLCONFIGDIR"] = "/data1/liuwenhao/tmp/mplconfig"
        env["XDG_CACHE_HOME"] = "/data1/liuwenhao/tmp/xdg-cache"
        if Path(DEFAULT_ROBOTWIN_ENV_BIN).exists():
            old_path = env.get("PATH", "")
            if DEFAULT_ROBOTWIN_ENV_BIN not in old_path.split(":"):
                env["PATH"] = f"{DEFAULT_ROBOTWIN_ENV_BIN}:{old_path}" if old_path else DEFAULT_ROBOTWIN_ENV_BIN
        prepend_env_path(env, "LD_LIBRARY_PATH", DEFAULT_SAPIEN_OIDN_LIBRARY)
    ...
    if any(code != 0 for code in exit_codes):
        print(f"[rpy_grid_rpy_rotation_shards] worker exit codes: {exit_codes}")
        return 1
    print(
        f"[rpy_grid_rpy_rotation_shards] complete episodes=[{start}, {end}) "
        f"arm_filter={args.arm_filter} save_path={save_path}"
    )
    return 0
